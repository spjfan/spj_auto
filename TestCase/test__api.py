...
from Utils import reqUtils, mysqlUtil,excelUtil


class TestGetApi:
    # def get_excel_data():
    #     case_list = excelUtil.ExcelUtil().get_caseList_from_excel()
    #     return case_list
    ...
    @pytest.mark.parametrize("caseData",excelUtil.ExcelUtil().get_caseList_from_excel())
    def test_get_excel_data(self,caseData):
        resp = reqUtils.ReqUtils().all_request(url=caseData["url"], method=caseData["method"])
        print(caseData["url"], "+++++++++++++++++++++++++-----------------------", resp.status_code)
        assert resp.status_code == 200
    ...
    def test_get_from_database(self):
        data = mysqlUtil.MysqlUtil().get_requests_form_database("test_case", "method", "url", 4)
        print(data)
        for i in data:
